...
const AllProduct = () => {
  ...
  const deleteModal = useSelector((state) => state.product.deleteModal);
  const selectedEntity = useSelector((state) => state.product.selectedEntity);
  ...
  const patchLoading = useSelector((state) => state.product.patchLoading);
  const offerModal = useSelector((state) => state.product.offerModal);
  const scoreModal = useSelector((state) => state.product.scoreModal);
  ...
  const CustomPagination = () => (
    <>
      {!loadings.getProducts && productList?.length > 0 ? (
        <div className="datatable_pagination_wrapper">
          <ReactPaginate
            ...
            onPageChange={(page) => {
              ...
              getProducts(page.selected + 1, perPage);
            }}
            pageCount={totalPage}
            breakLabel="..."
          ...
          <div className="d-flex align-items-center">
            ...
            <Select
              ...
              onChange={(value) => {
                ...
                getProducts(1, value, "without_filter");
              }}
              value={perPage}
  ...
  return (
    <Fragment>
      ...
      <Breadcrumbs
        title="محصولات"
        data={[{ title: "داشبورد" }, { title: "محصولات" }]}
      />
      {/* filterbar */}
      <Filterbar
        filters={filters}
      ...
      />
      {/* loading */}
      {loadings.getProducts ? <CustomLoading /> : null}
      {/* datatable */}
      {!loadings.getProducts && (
        <>
            ...
            <DataTable
              noDataComponent={
                !loadings.getProducts ? (
                  ...
                  <div style={{ margin: "24px 0" }}>محصولی یافت نشد.</div>
                )
              }
              noHeader
              pagination
      ...
      <Confirm
        ...
        title={"آیا از حدف این محصول مطمئن هستید؟"}
        noAction={() => dispatch(setDeleteModal(null))}
        noColor={"secondary"}
        noTitle={"انصراف"}
        yesLoading={loadings.deleteProduct}
        yesAction={() => deleteProduct(selectedEntity)}
        yesColor={"danger"}
      ...
      <Confirm
        ...
        title={"تخصیص تخفیف به این محصول"}
