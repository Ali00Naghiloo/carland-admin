...
const AllSliders = () => {
  ...
  const { getSliders, deleteSlider, loadings, filters, setFilters } =
    useSliders();

  const slidersList = useSelector((state) => state.sliders.slidersList);
  const deleteModal = useSelector((state) => state.sliders.deleteModal);
  const selectedEntity = useSelector((state) => state.sliders.selectedEntity);
  const currentPage = useSelector((state) => state.sliders.currentPage);
  const totalPage = useSelector((state) => state.sliders.totalPage);
  const perPage = useSelector((state) => state.sliders.perPage);
  const patchLoading = useSelector((state) => state.sliders.patchLoading);
  ...
  const CustomPagination = () => (
    <>
      {!loadings.getSliders && slidersList?.length > 0 ? (
        <div className="datatable_pagination_wrapper">
          <ReactPaginate
            ...
            onPageChange={(page) => {
              ...
              getSliders(page.selected + 1, perPage);
            }}
            pageCount={totalPage}
            breakLabel="..."
          ...
          <div className="d-flex align-items-center">
            ...
            <Select
              ...
              onChange={(value) => {
                ...
                getSliders(1, value, "without_filter");
              }}
              isSearchable={false}
              value={perPage}
  ...
  return (
    <Fragment>
      ...
      <Breadcrumbs
        title="اسلایدر ها"
        data={[{ title: "داشبورد" }, { title: "اسلایدر ها" }]}
      />
      <Filterbar
        filters={filters}
            ...
            <DataTable
              noDataComponent={
                !loadings.getSliders ? (
                  ...
                  <div style={{ margin: "24px 0" }}>اسلایدری یافت نشد.</div>
                )
              }
              noHeader
              pagination
      ...
      {/* loading */}
      {loadings.getSliders ? <CustomLoading /> : null}
      {/* delete modal */}
      <Confirm
        visible={deleteModal}
        setVisible={setDeleteModal}
        title={"آیا از حدف این اسلایدر مطمئن هستید؟"}
        noAction={() => dispatch(setDeleteModal(null))}
        noColor={"secondary"}
        noTitle={"انصراف"}
        yesLoading={loadings.deleteSlider}
        yesAction={() => deleteSlider(selectedEntity)}
        yesColor={"danger"}
