...
export const columns = [
  ...
  },
  {
    ...
    minWidth: "180px",
    sortable: true,
    selector: (row) => row.userAgent.lastName,
    cell: (row) => {
      if (row?.userAgent?.firstName || row?.userAgent?.lastName) {
        return row.userAgent.firstName + " " + row?.userAgent?.lastName;
      } else {
        return "-----";
      }
    },
  },
  {
    ...
    cell: (row) => {
      if (row?.zipCode) {
        return formatHelper.toPersianString(row?.zipCode);
      } else {
        return "-----";
      }
    },
