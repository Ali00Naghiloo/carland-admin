...
const AllSourceNumber = () => {
  ...
  const sourceNumberList = useSelector(
    (state) => state.sourceNumber.sourceNumberList
  );
  const deleteModal = useSelector((state) => state.sourceNumber.deleteModal);
  const selectedEntity = useSelector(
    (state) => state.sourceNumber.selectedEntity
  );
  const currentPage = useSelector((state) => state.sourceNumber.currentPage);
  const totalPage = useSelector((state) => state.sourceNumber.totalPage);
  const perPage = useSelector((state) => state.sourceNumber.perPage);
  const patchLoading = useSelector((state) => state.sourceNumber.patchLoading);
  ...
  const CustomPagination = () => (
    <>
      {!loadings.getSourceNumbers && sourceNumberList?.length > 0 ? (
        <div className="datatable_pagination_wrapper">
          <ReactPaginate
            ...
            onPageChange={(page) => {
              ...
              getSourceNumbers(page.selected + 1, perPage);
            }}
            pageCount={totalPage}
            breakLabel="..."
          ...
          <div className="d-flex align-items-center">
            ...
            <Select
              ...
              onChange={(value) => {
                ...
                getSourceNumbers(1, value, "without_filter");
              }}
              isSearchable={false}
              value={perPage}
  ...
  return (
    <Fragment>
      ...
      <Breadcrumbs
        title="شماره تلفن ها"
        data={[{ title: "داشبورد" }, { title: "شماره تلفن ها" }]}
      />
      {/* filterbar */}
      <Filterbar
        filters={filters}
      ...
      />
      {/* loading */}
      {loadings.getSourceNumbers ? <CustomLoading /> : null}
      {/* datatable */}
      {!loadings.getSourceNumbers && (
        <>
      ...
      <Confirm
        ...
        title={"آیا از حدف این شماره تلفن مطمئن هستید؟"}
        noAction={() => dispatch(setDeleteModal(null))}
        noColor={"secondary"}
        noTitle={"انصراف"}
        yesLoading={loadings.deleteSourceNumber}
        yesAction={() => deleteSourceNumber(selectedEntity)}
        yesColor={"danger"}
...
export default AllSourceNumber;
